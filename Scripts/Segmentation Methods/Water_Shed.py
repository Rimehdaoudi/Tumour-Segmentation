...
class Water_Shed(object):
    ...
    def highPassFilter(self):
        ...
        plt.subplot(122)
        plt.gca().set_title("High Pass Filtered"), plt.imshow(self.highpass_3x3, cmap = 'gray')
        plt.xticks([]), plt.yticks([])
        plt.show()
    ...
    def medianFilter(self, sigma):
        ...
        plt.subplot(121)
        plt.gca().set_title("Original"), plt.imshow(self.images[0], cmap = 'gray')
        plt.xticks([]), plt.yticks([])

        plt.subplot(122)
        plt.gca().set_title("Median Blur"), plt.imshow(self.median_array[0], cmap = 'gray')
        plt.xticks([]), plt.yticks([])
        plt.show()
    ...
    def thresholdSegment(self):
        ...
        for i in range(len(self.median_array)):
            self.retval, self.thresh = cv2.threshold(self.median_array[i], 180, 255, cv2.THRESH_BINARY)
            self.thresh_array.append(self.thresh)

        plt.subplot(121)
        plt.gca().set_title("Original"), plt.imshow(self.median_array[0], cmap = 'gray')
        plt.xticks([]), plt.yticks([])

        plt.subplot(122)
        plt.gca().set_title("Thresholded"), plt.imshow(self.thresh_array[0], cmap = 'gray')
        plt.xticks([]), plt.yticks([])
        plt.show()
    ...
    def waterShed(self):
        ...
        for i in range(len(self.thresh_array)):
            self.opening = cv2.morphologyEx(self.thresh_array[i], cv2.MORPH_OPEN, self.kernel, iterations = 2)
            ...
            self.ret, self.sure_fg = cv2.threshold(self.dist_transform, 0.7 * self.dist_transform.max(), 255, 0)
        ...
        plt.subplot(122)
        plt.gca().set_title("Watershed"), plt.imshow(self.markers_array[0], cmap = 'gray')
        plt.xticks([]), plt.yticks([])
        plt.show()
    ...
    def morphOperation(self):
        ...
        for i in range(len(self.markers_array)):
            ...
            self.erode = cv2.erode(self.thresh_array[i], self.kernel, iterations = 2)
        ...
        plt.subplot(122)
        plt.gca().set_title("Morphed"), plt.imshow(self.morph_array[0], cmap = 'gray')
        plt.xticks([]), plt.yticks([])
        plt.show()
    ...
    def writeSegment(self, OUTDIR):
        for i in range(len(self.morph_array)):
            cv2.imwrite(OUTDIR + "Result_" + str(i) + ".png", self.morph_array[i])
